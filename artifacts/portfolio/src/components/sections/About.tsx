...
function TiltCard({ feature, i }: { feature: typeof FEATURES[0]; i: number }) {
  ...
  return (
    <motion.div
      ...
      className={`p-8 rounded-2xl glass-panel border border-white/5 hover:border-primary/30 transition-colors group cursor-default ${feature.span ? "md:col-span-2" : ""}`}
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
            ...
            >
              {STATS.map(({ value, label, color }, i) => (
                <motion.div
                  ...
                  className="glass-panel rounded-2xl px-6 py-4 border border-white/5 text-center min-w-[100px] cursor-default"
                >
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {FEATURES.map((feature, i) => (
              <TiltCard key={feature.title} feature={feature} i={i} />
            ))}
          </div>
